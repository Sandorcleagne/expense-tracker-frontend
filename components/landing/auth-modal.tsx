...
export function AuthModal({
    ...
    try {
      if (mode === "login") {
        ...
        const userObj = {
          ...
          fullName: res?.data?.result?.userDetails?.fullName,
          avatar: res?.data?.result?.userDetails?.avatar || "",
          id: res?.data?.result?.userDetails?._id,
        };
        setUser(userObj);
      } else {
  ...
  const isLoginValid = email.trim() !== "" && password.trim().length >= 6;
        ...
        {success ? (
          ...
          <form onSubmit={handleSubmit} className="mt-4 space-y-4">
            ...
            <button
              ...
              className="inline-flex w-full items-center justify-center gap-2 rounded-lg bg-blue-600 px-4 py-2.5 text-sm font-medium text-white transition-colors hover:bg-blue-700 disabled:cursor-not-allowed disabled:opacity-50 cursor-pointer"
            >
